...
constexpr f32 UPDATE_TIME = 0.01;
...
i32 main() {
    ...
    for (u16 x = 0; x < CANVAS_WIDTH; x++) {
        for (u16 y = 0; y < CANVAS_HEIGHT; y++) {
            ...
            canvas[x * CANVAS_HEIGHT + y] = dist(rng);
        }
    }
    ...
    while (!WindowShouldClose()) {
        ...
        SetWindowTitle(std::format("Sand Simulator | FPS: {:.5}", deltaTime).c_str());
    }
